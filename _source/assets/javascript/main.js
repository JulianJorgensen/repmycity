...
var state_city_object = {"Alabama":["Autauga" ,"Baldwin" ,"Barbour" ,"Bibb" ,"Blount" ,"Bullock" ,"Butler" ,"Calhoun" ,"Chambers" ,"Cherokee" ,"Chilton" ,"Choctaw" ,"Clarke" ,"Clay" ,"Cleburne" ,"Coffee" ,"Colbert" ,"Conecuh" ,"Coosa" ,"Covington" ,"Crenshaw" ,"Cullman" ,"Dale" ,"Dallas" ,"DeKalb" ,"Elmore" ,"Escambia" ,"Etowah" ,"Fayette" ,"Franklin" ,"Geneva" ,"Greene" ,"Hale" ,"Henry" ,"Houston" ,"Jackson" ,"Jefferson" ,"Lamar" ,"Lauderdale" ,"Lawrence" ,"Lee" ,"Limestone" ,"Lowndes" ,"Macon" ,"Madison" ,"Marengo" ,"Marion" ,"Marshall" ,"Mobile" ,"Monroe" ,"Montgomery" ,"Morgan" ,"Perry" ,"Pickens" ,"Pike" ,"Randolph" ,"Russell" ,"Shelby" ,"St. Clair" ,"Sumter" ,"Talladega" ,"Tallapoosa" ,"Tuscaloosa" ,"Walker" ,"Washington" ,"Wilcox" ,"Winston"],

"Alaska":["Anchorage" ,"Bethel" ,"Bristol Bay" ,"Dillingham" ,"Fairbanks North Star" ,"Haines" ,"Juneau" ,"Kenai Peninsula" ,"Ketchikan Gateway" ,"Kodiak Island" ,"Matanuska-Susitna" ,"Nome" ,"North Slope" ,"Prince of Wales-Outer Ketchikan","Sitka" ,"Skagway-Hoonah-Angoon" ,"Southeast Fairbanks" ,"Valdez-Cordova" ,"Wade Hampton" ,"Wrangell-Petersburg" ,"Yakutat" ,"Yukon-Koyukuk"] ,
...
"Maryland":["Allegany","Anne Arundel","Baltimore City","Baltimore","Calvert","Caroline","Carroll","Cecil","Charles","Dorchester","Frederick","Garrett","Harford","Howard","Kent","Montgomery","Prince George's","Queen Anne's","Somerset","St. Mary's","Talbot","Washington","Wicomico","Worcester"],
...
"Missouri":["Adair","Andrew","Atchison","Audrain","Barry","Barton","Bates","Benton","Bollinger","Boone","Buchanan","Butler","Caldwell","Callaway","Camden","Cape Girardeau","Carroll","Carter","Cass","Cedar","Chariton","Christian","Clark","Clay","Clinton","Cole","Cooper","Crawford","Dade","Dallas","Daviess","Dent","DeKalb","Douglas","Dunklin","Franklin","Gasconade","Gentry","Greene","Grundy","Harrison","Henry","Hickory","Holt","Howard","Howell","Iron","Jackson","Jasper","Jefferson","Johnson","Knox","Laclede","Lafayette","Lawrence","Lewis","Lincoln","Linn","Livingston","Macon","Madison","Maries","Marion","McDonald","Mercer","Miller","Mississippi","Moniteau","Monroe","Montgomery","Morgan","New Madrid","Newton","Nodaway","Oregon","Osage","Ozark","Pemiscot","Perry","Pettis","Phelps","Pike","Platte","Polk","Pulaski","Putnam","Ralls","Randolph","Ray","Reynolds","Ripley","Saline","Schuyler","Scotland","Scott","Shannon","Shelby","St. Charles","St. Clair","St. Francois","St. Louis city","St. Louis","Ste. Genevieve","Stoddard","Stone","Sullivan","Taney","Texas","Vernon","Warren","Washington","Wayne","Webster","Worth","Wright"],
...
function make_state_list(){
    ...
    var count = 3;
    var i;
    for(i = 0; i < state_abbr_array.length; i++) {
        var state_name = state_abbr_array[i].name;
        ustate_name = state_name.toUpperCase();
        lstate_name = state_name.toLowerCase();
        out += '<li><a href="/collections/all/'+lstate_name+'">' + ustate_name +'<span> ('+count+') </span>' +'</li>';
    }
    out = out + "</ul>";
    document.getElementById("statelist").innerHTML = out;
}
...
function get_all_city_in_state(state){

    var city_arry=(state_city_object[state]);
    if(city_arry && is_array(city_arry))
    {
        console.log("Found");
        return city_arry;
    } else  {
        console.log("Not found the state");
    }
}

function check_city_in_state(state, city)
{
    var cities = get_all_city_in_state(state);
    if (!cities)
        return;

    var upcites = cities.map(function(value){
        return value.toUpperCase();
    });

    var pos = upcities.indexOf(city.toUpperCase());
...
function get_abbr_of_state(state){
    
    var i;
    var found='';

    for(i = 0; i < state_abbr_array.length; i++) {
        if (state_abbr_array[i].name == state)
        {
            found= state_abbr_array[i].abbreviation;
            break;
        }
    }
    if(found)
...
function get_state_of_city(city){

    var state_name='';
    var length = get_obj_length(state_city_object);
    
    for (var state in state_city_object)
    {
        var city_array = state_city_object[state];
        var upcity_array = city_array.map(function(value){return value.toUpperCase();});
        var check = upcity_array.indexOf(city.toUpperCase());
        if( check != -1)
        {
            state_name = state;
            break;
...
function get_abbr_of_city(city){

    var state = get_state_of_city(city);
    var state_abbr=get_abbr_of_state(state);
    console.log(state_abbr);
    return state_abbr;
}

function show_city_with_abbr(city){
    var abbr = get_abbr_of_city(city);
    var city_with_abbr = '<p>'+city.toUpperCase()+', '+ abbr+'</p>';
    document.getElementById("city_abbr_name").innerHTML= city_with_abbr;
}

$(function(){
    $("#city-state-list").hover( 
        function(){
